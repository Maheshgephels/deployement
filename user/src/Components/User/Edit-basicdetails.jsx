...
import { required, email, NAME, Img, PDF, option, number, username1, password,expiryDate } from '../Utils/validationUtils';
...
const EditBasicUserDetails = () => {
    ...
    const fetchFields = async () => {
        try {
            const token = getToken();
            const response = await axios.get(`${BackendAPI}/userdashboard/getBasicField`, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
    ...
    const onSubmit = async (formData) => {
        ...
        try {
            ...
            const token = getToken();
            await axios.post(`${BackendAPI}/userdashboard/updateBasicUser`, values, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
                                                ...
                                                {fieldLabels.map((label, index) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                           ...
                                                           >
                                                               {({ input, meta }) => {
                                                                   // Find the option corresponding to User.cs_title
                                                                   const selectedOption = prefixes.find(option => option.cs_prefix === User.cs_title);
                                                                   console.log('selectedOption', selectedOption);
                                                                   const formattedSelectedOption = selectedOption ? { value: selectedOption.cs_prefix, label: selectedOption.cs_prefix } : null;
                                                                   return (
                                                                    <div>
                                                                      ...
                                                                      <Select
                                                                        ...
                                                                        isSearchable={true}
                                                                        onChange={(value) => input.onChange(value.value)} // Handle change correctly
                                                                        onBlur={input.onBlur}
                                                                        classNamePrefix="react-select"
                                                                        value={formattedSelectedOption} // Set the selected option correctly
                                                                      />
                                                            ...
                                                            {fieldType[index] === 'Dropdown' && fieldLabels[index] === 'Title' && (
                                                                ...
                                                                >
                                                                    {({ input, meta }) => {
                                                                        ...
                                                                        // Map prefixes to create the options array
...
{fieldType[index] === 'Date' && (
                                                            ...
                                                            {fieldType[index] === 'Dropdown' && fieldLabels[index] === 'Workshop Category' && (
                                                                <Field
                                                                    name={`${fieldName[index]}`} // Use dynamic field name
                                                                    initialValue={Data.cs_workshop_category || ''}
                                                                    validate={requiredfield[index] === '1' ? composeValidators(option) : (value) => composeValidators()(value)}
                                                                >
                                                                    {({ input, meta }) => {
                                                            ...
                                                            {fieldType[index] === 'Dropdown' && fieldLabels[index] === 'Registration Category' && (
                                                                <Field
                                                                    name={`${fieldName[index]}`} // Use dynamic field name
                                                                    initialValue={Data.cs_reg_category || ''}
                                                                    validate={requiredfield[index] === '1' ? composeValidators(option) : (value) => composeValidators()(value)}
                                                                >
                                                                    {({ input, meta }) => {
                                                                        const selectedOption = regCat.find(option => option.cs_reg_cat_id === Data.cs_reg_category);
                                                                        ...
                                                                        let options = regCat.map(pref => ({
                                                                            value: pref.cs_reg_category,
                                                                            label: pref.cs_reg_category,
                                                                            id: pref.cs_reg_cat_id
                                                                        }));
                                                            ...
                                                            {/* {fieldType[index] === 'Dropdown' && fieldLabels[index] === 'Registration Type' && (
                                                                <Field
                                                                    name={`${fieldName[index]}`} // Use dynamic field name
                                                                    initialValue={Data.cs_reg_type || ''}
                                                                    validate={isFieldRequired ? composeValidators(required) : undefined}
                                                                >
                                                                    {({ input, meta }) => {
                                                                        // Convert Data.cs_reg_type to number
                                                                        const regType = parseInt(Data.cs_reg_type);
                                                ...
                                                <div>
                                                    <Button color='primary' type='submit' className="me-3 mt-3">Update User</Button>
...
export default EditBasicUserDetails;