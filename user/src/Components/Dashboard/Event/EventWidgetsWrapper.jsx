...
const EventWidgetsWrapper = () => {
  ...
  const handleViewDetails = (sortBy) => {
    navigate(`${process.env.PUBLIC_URL}/manage-faculty/Consoft`, { state: { sortBy, Title: "Total Faculty" } });
  };

  const handleUserViewDetails = () => {
    navigate(`${process.env.PUBLIC_URL}/manage-user/Consoft`, { state: { Title: "Total Users" } });
  };

  const handleloggedinViewDetails = () => {
    navigate(`${process.env.PUBLIC_URL}/Loggedin-user/Consoft`);
  };

  return (
    <>
      <CustomBreadcrumbs mainTitle="Statistics" />
      <Row>
        {/* User Count Card */}
