...
router.get('/getCategory', verifyToken, async (req, res) => {
  try {
    ...
    const { page = 1, pageSize = 10, search = '', sortColumn = 'cs_reg_cat_id', sortOrder = 'DESC' } = req.query;
    const offset = (page - 1) * pageSize;

    const validColumns = ['cs_reg_cat_id', 'cs_reg_category'];  // Add all valid column names here
...
//         END
//       ),
//       -- Add the new key dynamically if it doesn't exist
//       b.cs_badge_data = CASE 
//         WHEN JSON_CONTAINS_PATH(b.cs_badge_data, 'one', CONCAT('$.', ?, '_status')) THEN b.cs_badge_data
//         ELSE JSON_SET(b.cs_badge_data, CONCAT('$.', ?, '_status'), 0)
//       END
//       WHERE b.cs_reg_cat_id = ? AND b.cs_regno = ?;
//     `;
...
//       facilityName, facilityName, String(updatedAllowCount), facilityName,
//       facilityType, String(updatedAllowCount), facilityName, String(updatedAllowCount),
//       facilityType, facilityType, String(catId), String(cs_regno)
//     ];
...
async function processBadgeDataChunk(chunk, id, catId, facilityName, facilityId, facilityType, updatedAllowCount) {
  console.log("additionalFields",chunk);
  const updateQueries = chunk.map(async (badgeEntry) => {
    ...
    if (cs_reg_type && cs_reg_type !== "101") {
      ...
      if (facilityType === 'workshop') {
        ...
        const workshopId = workshopRows[0]?.cs_workshop_id;
        console.log(`workshop id`, workshopId);  // Log the field and its value for debugging

        // if (workshopId === workshop) {
          if (Object.values(additionalFieldValues).includes(workshopId)) {
          console.log(`updating`);  // Log the field and its value for debugging
...
router.post('/getCategoryAccess', verifyToken, async (req, res) => {
  try {
    ...
    // Step 1: Check if the `catId` exists in the `cs_os_facility_category` table
    const checkQuery = `
      SELECT * 
      FROM cs_os_facility_category 
      WHERE cs_reg_cat_id = ? 
    ...
    const [existingCategoryData] = await pool.query(checkQuery, [catId]);
    ...
    if (existingCategoryData.length > 0) {
      ...
      const fetchNewDataQuery = `
        SELECT ${columnsToFetch.join(',')}
        FROM cs_os_facility_category 
        LEFT JOIN cs_os_facility_detail ON cs_os_facility_category.cs_facility_detail_id = cs_os_facility_detail.cs_facility_detail_id
        LEFT JOIN cs_os_facilitytype ON cs_os_facility_detail.cs_facility_id = cs_os_facilitytype.cs_facility_id
        WHERE cs_os_facility_category.cs_reg_cat_id = ? 
        AND cs_os_facility_category.cs_status = 1
        AND cs_os_facilitytype.cs_status = 1;
      `;

      const [newCategoryData] = await pool.query(fetchNewDataQuery, [catId]);
      return res.json(newCategoryData);
    } else {
      // Step 2: Fetch all active facility IDs from `cs_os_facility_detail`
      const facilityIdQuery = `
        SELECT cs_facility_detail_id 
        FROM cs_os_facility_detail 
        WHERE cs_status = 1;
      `;
      ...
      const facilityIds = facilityIdResult.map(row => row.cs_facility_detail_id);

      // Step 3: Insert the category data for each facility if `catId` does not exist
      for (const facilityId of facilityIds) {
        const insertFacilityCategoryQuery = `
          INSERT INTO cs_os_facility_category 
          (cs_facility_detail_id, cs_reg_cat_id, cs_allow_count, cs_status)
          VALUES (?, ?, 0, 1);
        `;
        await pool.query(insertFacilityCategoryQuery, [facilityId, catId]);
      }

      // Step 4: Fetch and return the newly inserted data
      const columnsToFetch = [
        'cs_os_facility_category.*',
        'cs_os_facilitytype.cs_display_name',
        'cs_os_facility_detail.cs_facility_name'
...
router.put('/updateFacilityCount', verifyToken, async (req, res) => {
  try {
    const { cs_regno, cs_allow_count, facilityName, cs_user_status: cs_user_status_str } = req.body;
    const adminUsername = "admin";
    ...
    // Parse cs_user_status as an integer
    const cs_user_status = parseInt(cs_user_status_str, 10);

    if (rows[0].currentCount !== cs_user_status) {
      const insertStatusLogQuery = `
        INSERT INTO cs_os_reset_facility_log (cs_regno, cs_facility_type, cs_old_value, cs_new_value, cs_admin_username)
        VALUES (?, ?, ?, ?, ?)
      `;
      await pool.query(insertStatusLogQuery, [cs_regno, facilityName, rows[0].currentCount, cs_allow_count, adminUsername]);
    }
...
router.put('/resetFacility', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const updatedCsUserStatus = (cs_user_status - 1).toString();
...
router.post('/addcategory', verifyToken, async (req, res) => {
  try {
    ...
    const facilityIdQuery = `SELECT cs_facility_detail_id FROM cs_os_facility_detail WHERE cs_status = 1`;
    const facilityIdResult = await pool.query(facilityIdQuery);
...
router.post('/updatecategory', verifyToken, async (req, res) => {
  try {
    ...
    const { categoryName, registration, onsite, catId } = req.body;
  ...
  `;
    ...
    await pool.query(updateQuery, [categoryName, registration, onsite, catId]);
...
//     const selectUsersQuery = `SELECT cs_regno,cs_reg_type,cs_workshop_category FROM cs_os_users WHERE cs_reg_cat_id = ?`;
//     const [users] = await pool.query(selectUsersQuery, [Id]);
...
router.put('/UpdateStatus', queueMiddleware, verifyToken, async (req, res) => {
  try {
    ...
    const userQueryFields = ['cs_regno', 'cs_reg_type', 'cs_workshop_category', ...additionalFields].join(', ');




    // Retrieve users from cs_os_users where cs_reg_cat_id = Id
    const selectUsersQuery = `SELECT ${userQueryFields} FROM cs_os_users WHERE cs_reg_cat_id = ?`;
    const [users] = await pool.query(selectUsersQuery, [Id]);
    ...
    if (status === 0) {
      ...
      for (const user of users) {
        ...
        // Loop through each field in additionalFields to retrieve its userData
        additionalFields.forEach(field => {
          additionalFieldValues[field] = parseInt(user[field], 10);
        });
...
router.get('/genratebadgedata', verifyToken, async (req, res) => {
  try {
    console.log("Generating badge data...");

    // Step 1: Fetch users who need badge creation
    const [users] = await pool.query(`
            SELECT * FROM cs_os_users WHERE cs_isconfirm = 1 AND cs_isbadge_created = 0
        `);

    // Prepare to hold badge data for each user
    const badgeDataList = [];

    for (const userData of users) {
      const regId = userData.cs_reg_cat_id;
      ...
      // Fetch facility data based on registration ID
      const [rows] = await pool.query(`
                SELECT fd.cs_facility_name, fc.cs_allow_count, fd.cs_facility_id
                FROM cs_os_facility_category fc
                JOIN cs_os_facility_detail fd ON fc.cs_facility_detail_id = fd.cs_facility_detail_id
      ...
      // Initialize badge data object for the current user
      const badgeData = {};
      ...
      // Loop through each field in additionalFields to retrieve its userData
      additionalFields.forEach(field => {
        additionalFieldValues[field] = parseInt(userData[field], 10); // Converts the value to an integer
      });
      
      console.log("additionalFieldValues", additionalFieldValues);

      // Check if user registration type is available and not 101
      if (userData.cs_reg_type && userData.cs_reg_type !== 101) {
        console.log("I am in daywise");
        const reg_daytype = days; // Assuming 'days' is defined somewhere in your code
        ...
        if (rows) {
          ...
          for (const row of rows) {
            ...
            const facilityTypeQuery = `
                            ...
                            WHERE cs_facility_id = ?
                        `;
            const [typeRows] = await pool.query(facilityTypeQuery, [facilityId]);
            const facilityType = typeRows[0]?.cs_type;
            ...
            if (facilityType === 'workshop') {
              const workshopQuery = `
                                ...
                                WHERE cs_facility_id = ? 
                            `;
              const [workshopRows] = await pool.query(workshopQuery, [facilityId]);
              const workshopId = workshopRows[0]?.cs_workshop_id;
              ...
              // Facility doesn't match, set count based on day type
              if (facilityName.includes(reg_daytype) || !/\d$/.test(facilityName)) {
                badgeData[facilityName] = allowCount;
              } else {
                badgeData[facilityName] = "0";
              }
              // Set the status for each facility
              badgeData[facilityName + '_status'] = "0";
            }
          }
        } else {
        ...
        // cs_reg_type not available or equals 101, set count to 0 for all facilities
        ...
        if (rows) {
          for (const row of rows) {
            ...
            if (facilityType === 'workshop') {
              ...
              // If facility type is not workshop, set count to allowCount
              badgeData[facilityName] = allowCount;
              badgeData[facilityName + '_status'] = "0";
            }
          }
      ...
      badgeDataList.push({
        regNo: userData.cs_regno, // Assuming you have a registration number field
        regCatId: regId,
        badgeData: JSON.stringify(badgeData) // Store badge data as JSON
      });
    }

    // Step 4: Insert badge data into cs_os_badges
    for (const badgeDataEntry of badgeDataList) {
      const insertBadgeQuery = `
                INSERT INTO cs_os_badges (cs_regno, cs_reg_cat_id, cs_badge_data)
                VALUES (?, ?, ?)
            `;
      await pool.query(insertBadgeQuery, [badgeDataEntry.regNo, badgeDataEntry.regCatId, badgeDataEntry.badgeData]);

      const updateUserQuery = `
            UPDATE cs_os_users 
            SET cs_isbadge_created = 1 
            WHERE cs_regno = ?
        `;
      await pool.query(updateUserQuery, [badgeDataEntry.regNo]);
    }

    res.status(200).json({ message: 'Badge data generated successfully', badgeData: badgeDataList });
  } catch (error) {
