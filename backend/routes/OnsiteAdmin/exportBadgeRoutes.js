...
router.post('/exportbadgeUsers', verifyToken, async (req, res) => {
    try {
        ...
        if (!selectedCategory || !csFieldNames || csFieldNames.length === 0) {
            return res.status(400).json({
                success: false,
                message: 'Selected category and field names are required',
            });
        }

        // Filter out empty or invalid field names
        const filteredFieldNames = csFieldNames.filter(fieldName => fieldName.trim() !== '');
        ...
        let selectFields = [...filteredFieldNames];
        let joins = '';

        // Add 'fullname' dynamically
        if (filteredFieldNames.includes('fullname')) {
            selectFields = selectFields.filter(fieldName => fieldName !== 'fullname');
            selectFields.push(`CONCAT(IFNULL(u.cs_title, ''), ' ', u.cs_first_name, ' ', u.cs_last_name) AS fullname`);
        }

        // Add join for `cs_reg_type` if needed
        if (filteredFieldNames.includes('cs_reg_type')) {
            selectFields = selectFields.map(field =>
                field === 'cs_reg_type' ? 'rt.cs_reg_daytype_name AS cs_reg_type' : field
            );
            joins += `
                LEFT JOIN cs_os_reg_daytype rt ON u.cs_reg_type = rt.cs_reg_daytype_id
            `;
        ...
        // Add join for `cs_workshop_category` if needed
        if (filteredFieldNames.includes('cs_workshop_category')) {
            selectFields = selectFields.map(field =>
                field === 'cs_workshop_category' ? 'w.cs_workshop_name AS cs_workshop_category' : field
            );
            joins += `
                LEFT JOIN cs_os_workshop w ON u.cs_workshop_category = w.cs_workshop_id
            `;
        ...
        // Add event day filter if provided
        if (Array.isArray(selectedEventDay) && selectedEventDay.length > 0) {
            console.log('Selected Event Day:', selectedEventDay);
            query += ` AND u.cs_reg_type = ?`;
            queryParams.push(selectedEventDay[0]); // Assuming one event day for simplicity
        }
