...
`;
    ...
    res.json({ Fields: formData, prodData });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
router.post('/editField', async (req, res) => {
  ...
  try {
    ...
    const { id, label, onspot, required, option_data, deletedOptionIds, new_option, status } = req.body;
    ...
    const updateQuery = `
      UPDATE cs_os_field_data 
      SET cs_field_label = ?, cs_visible_onspot = ?, cs_is_required = ?, cs_status = ?
      WHERE cs_field_id = ?`;

    // Execute the query to update the fields
    await pool.query(updateQuery, [label, onspot, required, status, id]);
...
router.post('/CreateCustomField', verifyToken, async (req, res) => {
  try {
    // Extract role_id from the request body
    const { customLabel, FieldType, options } = req.body;
    const status = 1;
    const spot = 0;
    ...
    // Construct the SQL query to alter the table based on FieldType
    let alterTableQuery;

    if (FieldType === 6) {
      // For FieldType = 6, add a DATE type column before cs_status
      alterTableQuery = `
    ALTER TABLE cs_os_users
    ADD COLUMN ${fieldName} DATE NULL 
    AFTER cs_status
  `;
    } else if (FieldType !== 4) {
      // For other FieldTypes except 4, add a varchar(255) column
      alterTableQuery = `
    ALTER TABLE cs_os_users
  ...
  `;
      ...
      // For FieldType = 4, add a varchar(255) NOT NULL column with default value 'No'
      alterTableQuery = `
    ALTER TABLE cs_os_users
    ADD COLUMN ${fieldName} varchar(255) NOT NULL DEFAULT 'No'
    AFTER cs_status
  `;
    }
    ...
    const insertQuery = `
    INSERT INTO cs_os_field_data (cs_field_name, cs_field_label, cs_is_required, cs_field_type, cs_field_order, cs_visible_add_user, cs_visible_onspot, cs_iscustom, cs_status, cs_visible_reg_userform, cs_visible_reg_adminform, cs_visible_reg_basicform, cs_visible_reg_confirmform)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`;

    // Execute the query to insert the new role into the cs_tbl_roles table
    await pool.query(insertQuery, [fieldName, customLabel, spot, FieldType, newfieldOrder, spot, spot, 1, status, 0, 0, 0, 0]);

    // Get the newly inserted field's ID
    const result = await pool.query(`SELECT cs_field_id FROM cs_os_field_data WHERE cs_field_label = ?`, [customLabel]);
    const fieldId = result[0][0].cs_field_id;


    // Construct the SQL query to insert options into the cs_os_field_option table
    const insertQuery1 = `
        INSERT INTO cs_os_field_option (cs_field_type, cs_field_id, cs_field_option, cs_field_option_value, cs_status)
        VALUES (?, ?, ?, ?, ?)
      `;
    ...
    await Promise.all(options.map(async (option) => {
      ...
      await pool.query(insertQuery1, [FieldType, fieldId, option, lowercaseOption, status]);
    }));
...
router.post('/deletefield', verifyToken, async (req, res) => {
  try {
    ...
    const deleteQuery = 'DELETE FROM cs_os_field_data WHERE cs_field_id = ?';
    await pool.query(deleteQuery, [id]);

    const deleteQuery1 = 'DELETE FROM cs_os_field_option WHERE cs_field_id = ?';
...
router.post('/moveup', verifyToken, async (req, res) => {
  try {
    ...
    SELECT cs_field_id 
    FROM cs_os_field_data 
    WHERE cs_field_order < ? AND cs_field_order > 1 
...
router.post('/movedown', verifyToken, async (req, res) => {
  try {
    ...
    const getMaxOrderQuery = 'SELECT MAX(cs_field_order) AS max_order FROM cs_os_field_data';
    ...
    const swapOrderQuery = 'SELECT cs_field_id FROM cs_os_field_data WHERE cs_field_order = ?';
...
router.get('/getCatId', verifyToken, async (req, res) => {
  try {
    ...
    let query1 = `
    SELECT cs_workshop_id,cs_workshoptype_id
    FROM cs_os_workshop
    WHERE cs_status = 1
  `;
  ...
  `;
    ...
    WHERE cs_status = 1
  `;
    const [Workshoptypedata] = await pool.query(query5);

    const fieldType13Query = `
    SELECT cs_field_name,cs_workshoptype_id,cs_field_label
    FROM cs_os_field_data
