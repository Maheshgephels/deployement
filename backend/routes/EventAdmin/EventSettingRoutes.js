...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    if (file.fieldname === 'logoimage') {
      ...
      uploadPath = 'eventbackgroundimage/'; // Save other files to 'faculty-profile' directory
    }
    cb(null, uploadPath);
  },
  filename: (req, file, cb) => {
    ...
    let filename;
    if (file.fieldname === 'logoimage') {
      filename = `${dName}-eventlogoimage${path.extname(file.originalname)}`;
    } else if (file.fieldname === 'Facultycv') {
      filename = `${dName}-facultycv${path.extname(file.originalname)}`;
    } else {
      filename = `${dName}-eventbackground${path.extname(file.originalname)}`;
    }
    cb(null, filename);
  },
});
...
router.get('/getAdminSetting', verifyToken, async (req, res) => {
  try {
    // Query to fetch cs_value for id 38 from cs_tbl_sitesetting table
    const query = "SELECT cs_parameter,cs_value FROM cs_tbl_sitesetting WHERE cs_parameter IN ('Time Zone', 'Event Days', 'Event Name', 'AdminEmail', 'mobile', 'From', 'CC', 'BCC','TO','Reply-To', 'SMS Sending', 'Spot Registration Start', 'Admin Reg Start Number','Event Start Date','event_venue','dynamiclogin_id','event_end_date','event_image_url','homepage_bg_img','event_time')";
...
async function processRequest() {
  ...
  const brightModeIcon = req.files['logoimage'] ? req.files['logoimage'][0] : null;
  const facultycv = req.files['backgroundimg'] ? req.files['backgroundimg'][0] : null;


  console.log("brightModeIcon", brightModeIcon);
  ...
  try {
    ...
    if (brightModeIcon) {
      await pool.query(updateQuery, [brightModeIcon.path, 'event_image_url']);
    }
    if (facultycv) {
      await pool.query(updateQuery, [facultycv.path, 'homepage_bg_img']);
    }
    ...
    if (brightModeIcon) {
      ...
      await pool2.query(updateEventDetailsQueryImage, [
        adminPath + brightModeIcon.path, // Concatenate AdminPath with the image path
    ...
    if (facultycv) {
      ...
      await pool2.query(updateEventDetailsQueryBackground, [
        adminPath + facultycv.path, // Concatenate AdminPath with the background image path
        updatedResults[0].cs_value // Use the actual event host identifier
      ]);
    }
