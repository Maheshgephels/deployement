...
router.get('/getAddon', verifyToken, async (req, res) => {
  try {
    ...
    const validColumns = ['addon_id ', 'addon_title', 'addon_visiblility', 'userCount', 'addon_ispaid', 'addon_type', 'addon_status'];  // Add all valid column names here
    const columnToSortBy = validColumns.includes(sortColumn) ? sortColumn : 'addon_id ';


    // const columnsToFetch = ['*'];
    const columnsToFetch = ['cs_reg_add_ons.*', 'COUNT(DISTINCT cs_os_users.id) AS userCount', 'cs_os_users.cs_addons'];
    ...
    let query = `
      ...
      FROM cs_reg_add_ons
            LEFT JOIN cs_os_users on cs_reg_add_ons.addon_id = cs_os_users.cs_addons
      WHERE 1
    `;
    ...
    // Append pagination
    query += `
        GROUP BY cs_reg_add_ons.addon_id
