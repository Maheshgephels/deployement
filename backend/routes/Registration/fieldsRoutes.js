...
//     // Filter out null values from the option_data array
//     const filteredOptionData = option_data.filter(option => option !== null);
...
//         await pool.query(updateOptionQuery, [filteredOption.optionVal, modifiedOption.optionVal, filteredOption.option_id]);
//       } else {
//         // Insert a new option
//         const insertOptionQuery = `
//           INSERT INTO cs_os_field_option (cs_field_id, cs_field_option, cs_field_option_value, cs_field_type) 
...
router.get('/getCatId', verifyToken, async (req, res) => {
  ...
  `;
    ...
    res.json({ catData, workshopData, mandatoryData, prefixData });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
