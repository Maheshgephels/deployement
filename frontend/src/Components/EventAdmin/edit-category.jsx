...
const EditEventCategory = () => {
    ...
    const { catId, catName, onsite, registration, prodData } = location.state;
    ...
    const handleNavigation = () => {
        navigate(`${process.env.PUBLIC_URL}/event/manage-event-category/Consoft`);
    };
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            const response = await axios.post(`${BackendAPI}/eventcategory/updatecategory`, {
                catId,
            ...
            SweetAlert.fire({
                ...
                if (result.dismiss === SweetAlert.DismissReason.timer) {
                    navigate(`${process.env.PUBLIC_URL}/event/manage-event-category/${layoutURL}`);
                }
            });
        } catch (error) {
    ...
    const EventcategoriesPermissions = permissions['Eventcategories'];
