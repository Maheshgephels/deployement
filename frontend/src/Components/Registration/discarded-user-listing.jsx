...
import { MdDelete, MdInfoOutline, MdOutlineRestore } from "react-icons/md";
...
const DiscardedUser = () => {
    ...
    const UserListingpermissions = permissions['DiscardedUser'];
    ...
    const getSettings = (user) => [
        ...
        // },
        {
            ...
            label: (
                <div onClick={() => openDuplicateModal(user)}>
                    <MdOutlineRestore /> Restore Entry
                </div>
            ),
        },
        // {
    ...
    return (
        <Fragment>
                                    ...
                                    {loading ? (
                                        ...
                                        <Table className="table">
                                            ...
                                            <tbody className='text-center'>
                                                {users
                                                    .filter(user => user.is_discarded === 0)  // Only include users where is_discarded is 1
                                                    .map((user, index) => (
            ...
            <Modal isOpen={duplicateModalOpen} toggle={closeDuplicateModal} allowOutsideClick='false' centered size="md">
                ...
                <ModalBody>
                    <div className='ms-2'>
                        <p>Are you sure you want to restore the payment entry for <strong>{userName}</strong>? This action will restore the associated payment entry, allowing further processing for this user.</p>
                    </div>
                </ModalBody>
                <ModalFooter>
