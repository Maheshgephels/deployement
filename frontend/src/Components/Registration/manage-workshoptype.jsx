...
const RegWorkshoptype = () => {
    ...
    const [deleteWorkshopDetails, setDeleteWorkshopDetails] = useState({ workshopId: null }); // New state for delete workshop details
    ...
    const WorkshopsPermissions = permissions['RegWorkshoptype'];
    ...
    const fetchWorkshop = async () => {
        try {
            ...
            //     }
            // });
            const Response = await axios.get(`${BackendAPI}/regWorkshop/getWorkshoptype`, {
                params: {
                    page: currentPage,
                    pageSize: pageSize,
                    search: searchText,
    ...
    const getSettings = (item) => [
        ...
        ...(WorkshopsPermissions?.edit === 1
            ? [{
                key: '1',
                label: (
                    <div onClick={() => handleEdit(item.id, item.workshoptype_name)}>
                        <FaEdit /> Edit Workshop Type
                    </div>
                ),
            }]
            : []
        ...
        ...(WorkshopsPermissions?.delete === 1
            ? [{
                ...
                label: (
                    <div onClick={item.workshop_count > 0
                        ? () => openWarningModal(item.workshoptype_name)
                        : () => openDeleteModal(item.id,item.workshoptype_name)
                    }>
    ...
    const handleEdit = (workshopId, workshopname) => {
        const URL = '/registration/edit-reg-workshoptype/';
    ...
    const handleDelete = async () => {
        try {
            ...
            const token = getToken();
            await axios.delete(`${BackendAPI}/regWorkshop/deleteworkshoptype`, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                },
                data: { workshopId, facilityId, workshopName } // Include the data payload correctly
            });
            SweetAlert.fire({
                title: 'Success!',
    ...
    const handleStatusUpdate = async () => {
        ...
        const currentWorkshop = data.find(workshop => workshop.id === workshopId);
        if (!currentWorkshop) return;
        const currentStatus = currentWorkshop.cs_status;
        const newStatus = currentStatus === 0 ? 1 : 0;
        try {
            const token = getToken();
            await axios.put(`${BackendAPI}/regWorkshop/UpdateworkshoptypeStatus`, { workshopId, status: newStatus, Name: currentWorkshop.cs_workshop_name }, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
            const updatedWorkshops = data.map((workshop) =>
                workshop.id === workshopId ? { ...workshop, cs_status: newStatus } : workshop
            );
    ...
    const openDeleteModal = (workshopId, workshopName) => {
        setDeleteWorkshopDetails({ workshopId});
        setWorkshopName(workshopName);
        setModal(true);
    };
    ...
    return (
        <Fragment>
                        ...
                        <Card>
                            ...
                            <CardHeader>
                                ...
                                <div className="mb-2 mb-md-0 w-100">
                                        ...
                                        <InputGroup style={{ width: '100%', maxWidth: '250px', borderRadius: '20px' }}>
                                            <InputGroupText>
                                                <FiSearch />
                                            </InputGroupText>
                                            <Input
                                                placeholder="Search Workshop Type"
                                                onChange={e => handleSearch(e.target.value)}
                            ...
                            <CardBody>
                                <div className='table-responsive'>
                                    {loading ? (
                                        ...
                                        <p className='text-center'>No workshop type found</p>
                                    ) : (
                                        <Table>
                                            <thead>
                                                <tr className='border-bottom-primary'>
                                                    <th scope='col' className='text-start'>{'Sr No.'}</th>
                                                    <th scope='col' className='text-start' onClick={() => handleSort('workshoptype_name')}>
                                                        {'Workshop Type Name'}
                                                        {getSortIndicator('workshoptype_name')}
                                                    </th>
                                                    {/* <th scope='col' className='text-center' onClick={() => handleSort('workshop_count')}>
                                                        {'Registered User Count'}
                                                        {getSortIndicator('workshop_count')}
                                                    </th> */}
                                            ...
                                            <tbody>
                                                {data.map((item, index) => (
                                                    <tr key={index} className="border-bottom-primary">
                                                        ...
                                                        <td className='text-start'>{item.workshoptype_name}</td>
                                                        {/* <td className='text-center'>{item.workshop_count}</td> */}
                                                        <td className='text-center'>
                                                            {item.cs_status === 0 ? (
                                                                <span
                                                                    style={{ color: 'red', fontSize: '20px', cursor: "pointer" }}
                                                                    onClick={item.workshop_count > 0 ? () => openWarningModal(item.workshoptype_name) : () => openStatusModal(item.id, item.workshoptype_name, item.cs_status)}
                                                                    data-tooltip-id="tooltip"
                                                                    data-tooltip-content={item.workshop_count > 0 ? "Cannot change status, users registered" : "Inactive status"}
                                                                ...
                                                                <span
                                                                    ...
                                                                    onClick={item.workshop_count > 0 ? () => openWarningModal(item.workshoptype_name) : () => openStatusModal(item.id, item.workshoptype_name, item.cs_status)}
                                                                    data-tooltip-id="tooltip"
                                                                    data-tooltip-content={item.workshop_count > 0 ? "Cannot change status, users registered" : "Active status"}
                                                                    data-tooltip-event="click focus"
                                                                >
                                                                    <GoDotFill />
                                                                </span>
                                                            ...
                                                            <td className='text-end'>
                                                                ...
                                                                {WorkshopsPermissions?.delete === 1 && (
                                                                    <Button color="danger" size="sm"
                                                                        onClick={item.workshop_count > 0 ? () => openWarningModal(item.cs_workshop_name) : () => openDeleteModal(item.cs_workshop_id, item.cs_facility_id, item.cs_workshop_name)}
                                                                        data-tooltip-id="tooltip"
                                                                        data-tooltip-content={item.workshop_count > 0 ? "Cannot delete, users registered" : "Delete Workshop"}
                                                                        data-tooltip-event="click focus"
                                                                    >
                                                                        <MdDelete />
                                                                    </Button>
            ...
            <Modal isOpen={isWarningModalOpen} toggle={closeWarningModal} centered size="md">
                ...
                <ModalBody>
                    <div className='ms-2'>
                        <p><strong>{workshopName}</strong> workshop type contains entered workshop. First,inactive or delete them.</p>
                    </div>
                </ModalBody>
                <ModalFooter>
...
export default RegWorkshoptype;
