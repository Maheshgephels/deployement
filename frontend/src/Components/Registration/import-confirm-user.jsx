...
const ImportConfirmRegUser = () => {
    ...
    const processFileData = (fileData) => {
        ...
        // Function to handle converting scientific notation to a string
        const normalizePhoneNumber = (phone) => {
            if (phone && typeof phone === 'string' && phone.includes('E+')) {
                const number = parseFloat(phone).toFixed(0);
                return number;
            }
            return phone;
        };
        ...
        const validateHeaders = (headers) => {
            const requiredHeaders = ['Registration Number', 'Title', 'First Name', 'Last Name', 'Phone', 'Email', 'Registration Category ID'];
        ...
        const validateNonEmptyFields = (data) => {
            const requiredFields = ['Registration Number', 'First Name', 'Last Name', 'Registration Category ID'];
            return data.filter(item =>
        ...
        fileData.forEach(item => {
            item['Phone'] = normalizePhoneNumber(item['Phone']);
        });
        ...
        const duplicateregnoInFile = checkForDuplicates(fileData, 'Registration Number');
        ...
        const duplicateregnoFromExistingUsers = fileData.filter(item => userRegnoSet.has(item['Registration Number']));
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                const sendChunk = async (chunk) => {
                    try {
                        await axios.post(`${BackendAPI}/reguser/addBulkConfirmUser`, {
                ...
                SweetAlert.fire({
                    ...
                    if (result.dismiss === SweetAlert.DismissReason.timer) {
                        navigate(`${process.env.PUBLIC_URL}/registration/import-confirm-withreg-user/Consoft`);
                    }
                });
            }
        } catch (error) {
    ...
    const handleDownloadSampleFile = async () => {
        try {
            ...
            console.log("hello");
            const response = await axios.get(`${BackendAPI}/reguser/Confirmsamplefilewithregno`, {
            ...
            link.href = url;
            link.setAttribute('download', 'sample_with_regno.csv');
    ...
    const handleDownloadInstructionFile = async () => {
        try {
            // Make a GET request to the sample file route
            console.log("hello");
            const response = await axios.get(`${BackendAPI}/reguser/ConfirmInstructionfilewithregno`, {
                responseType: 'blob' // Specify responseType as 'blob' to handle binary data
            });
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'Instruction_with_regno.csv');
            document.body.appendChild(link);
            link.click();
                ...
                <>
                    ...
                    >
                        <PopoverBody>
                            ...
                            First, download and review the instruction file, then download the sample file and enter your data. <br />
                            Once you have completed and verified your entries, select the file and click <strong>Import Data</strong> to proceed.<br />
                        ...
                        <Card>
                            <CardHeader className="d-flex justify-content-between align-items-center flex-column flex-md-row">
                                <div className="mb-2 mb-md-0">
                                    <h5 className="mb-2 text-start">Import Confirm User with Reg No</h5>
...
export default ImportConfirmRegUser;
