...
const BasicUserListing = () => {
    ...
    const fetchUsers = useCallback(async () => {
        try {
            ...
            let url = `${BackendAPI}/reguser/getBasicUser?page=${currentPage}&pageSize=${pageSize}&search=${searchText}&sortColumn=${sortColumn}&sortOrder=${sortOrder}&selectedColumns=${columnsParam}`;
    ...
    const UserListingpermissions = permissions['BasicUserListing'];

    useEffect(() => {
        const storedColumns = localStorage.getItem('basicColumns');
        if (storedColumns) {
    ...
    const handleColumnChange = (selectedOptions) => {
        ...
        localStorage.setItem('basicColumns', JSON.stringify(selectedOptions.map(option => option.value)));
    };
    ...
    const actions = (users) => [
        ...
        },
        {
            key: '3',
            label: (
                <div onClick={() => bulkoperation(2)}>Make Unduplicate</div>
            ),
        },
        {
            key: '4',
            label: (
                <div onClick={() => bulkoperation(3)}>Make Duplicate</div>
            ),
        },
        {
            key: '5',
                                            ...
                                            <tbody className='text-center'>
                                                {users.map((user, index) => (
                                                    <tr key={index} className="border-bottom-primary">
                                                        ...
                                                        <td className='text-center'>
                                                            {user.cs_isduplicate === "1" || user.cs_isduplicate === 1 ? (
                                                                // Duplicate status
                                                                <span
                                                                    style={{ color: 'orange', fontSize: '20px', cursor: "pointer" }}
                                                                    onClick={() => openDuplicateModal(user)}
                                                                    data-tooltip-id="tooltip"
                                                                    data-tooltip-content="Duplicate Status"
                                                                    data-tooltip-event="click focus"
                                                                >
                                                                    <GoDotFill />
                                                                </span>
...
export default BasicUserListing;
