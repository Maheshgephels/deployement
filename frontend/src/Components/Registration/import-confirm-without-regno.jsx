...
const ImportConfirmRegUserwithoutregno = () => {
    useAuth();
    ...
    const processFileData = (fileData) => {
        ...
        const validateNonEmptyFields = (data) => {
            const requiredFields = ['First Name', 'Last Name', 'Registration Category ID'];
            return data.filter(item =>
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                const sendChunk = async (chunk) => {
                    try {
                        await axios.post(`${BackendAPI}/reguser/addBulkConfirmUserwithputreg`, {
                ...
                SweetAlert.fire({
                    ...
                    if (result.dismiss === SweetAlert.DismissReason.timer) {
                        navigate(`${process.env.PUBLIC_URL}/registration/import-confirm-withoutreg-user/Consoft`);
                    }
                });
            }
        } catch (error) {
    ...
    const handleDownloadSampleFile = async () => {
        try {
            ...
            console.log("hello");
            const response = await axios.get(`${BackendAPI}/reguser/Confirmsamplefilewithoutregno`, {
            ...
            link.href = url;
            link.setAttribute('download', 'sample_without_regno.csv');
    ...
    const handleDownloadInstructionFile = async () => {
        try {
            // Make a GET request to the sample file route
            console.log("hello");
            const response = await axios.get(`${BackendAPI}/reguser/ConfirmInstructionfilewithoutregno`, {
                responseType: 'blob' // Specify responseType as 'blob' to handle binary data
            });
                        ...
                        <Card>
                            <CardHeader className="d-flex justify-content-between align-items-center flex-column flex-md-row">
                                <div className="mb-2 mb-md-0">
                                    <h5 className="mb-2 text-start">Import Confirm User without Reg No</h5>
...
export default ImportConfirmRegUserwithoutregno;
