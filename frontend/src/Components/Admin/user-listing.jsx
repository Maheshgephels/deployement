...
const UserListing = () => {
    ...
    const fetchUsers = useCallback(async () => {
        try {
            ...
            let url = `${BackendAPI}/manageuser/getUser?page=${currentPage}&pageSize=${pageSize}&search=${searchText}&sortColumn=${sortColumn}&sortOrder=${sortOrder}&selectedColumns=${columnsParam}`;
            if (catID) {
                url += `&catID=${catID}`;
            }
            if (tbl) {
    ...
    const handleColumnChange = (selectedOptions) => {
        ...
        localStorage.setItem('selectedColumns', JSON.stringify(selectedOptions.map(option => option.value)));
    };
...
//Certificate Printing End
    ...
    return (
        <Fragment>
            ...
            } parent="Manage User" title="User Listing" />
