...
const Fields = () => {
    ...
    const fetchFields = async () => {
        try {
            ...
            const response = await axios.get(`${BackendAPI}/field/getBasicFields`, {
                headers: {
    ...
    const fetchFormFields = async () => {
        try {
            ...
            // console.log("FORM DATA", response.data.Fields);
            const fieldData = response.data.Fields.map(item => ({
                id: item.cs_field_id,
                label: item.cs_field_label,
                required: item.cs_is_required === '1',
                status: item.cs_status === 2,
    ...
    const fetchData = async () => {
        try {
            const token = getToken();
            const response = await axios.get(`${BackendAPI}/field/getData`, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
    ...
    const updateLabel = async () => {
        if (selectedItem) {
            try {
                ...
                await axios.post(`${BackendAPI}/field/editfield`, data, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
    ...
    const fetchLockStatus = async () => {
        try {
            const token = getToken();
            const response = await axios.get(`${BackendAPI}/field/getLockStatus`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const toggleLock = async () => {
        try {
            ...
            const response = await axios.post(`${BackendAPI}/field/updateLockStatus`, { cs_value: newLockStatus }, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
