...
const EditCategory = () => {
    ...
    const { catId, catName, event, prodData } = location.state;
    ...
    const [showRemaining, setShowRemaining] = useState(event);
    ...
    const fetchCategory = async () => {
        try {
            ...
            setCategory(response.data || []); // Ensure category is always an array
    ...
    const onSubmit = async (values) => {
        ...
        try {
            const registration = values.show_reg ? 1 : 0;
            ...
            const response = await axios.post(`${BackendAPI}/category/updatecategory`, {
                ...
                registration: values.show_reg,
                onsite: values.show_onsite
                // spot: spotValue,
                                            ...
                                            {prodData && prodData.some(item => item.product_id === 3 && item.cs_status === 1) && (
                                                ...
                                                <Row>
                                                    <Col md="12" className="mb-3">
                                                        <Field
                                                            name="show_onsite"
                                                            initialValue={category[0]?.cs_show_spot_form || ''}
                                                            type="checkbox"
                                                        >
                                                            {({ input, meta }) => (
                                            ...
                                            {prodData && prodData.some(item => item.product_id === 1 && item.cs_status === 1) && (
                                                ...
                                                <Row>
                                                    <Col md="12" className="mb-3">
                                                        <Field
                                                            name="show_reg"
                                                            initialValue={category[0]?.cs_show_conference_form || ''}
                                                            type="checkbox"
                                                        >
                                                            {({ input, meta }) => (
                                                                <div className="d-flex align-items-center mb-2">
